...
from .utils import mm_to_px, stickers, draw_circle, get_wrapped_text, A4_WIDTH, A4_HEIGHT, FONT_HEIGHT
...
def draw_text(draw, i: int, j: int, text: str, font) -> None:
    j_offset = -120
    for line in get_wrapped_text(text=text, font=font, line_length=MAX_LINE_LENGTH):
        ...
        j_offset += FONT_HEIGHT


def draw_text_discs(t: hash, language_code: str, font, with_border: bool) -> None:
    page = Image.new("RGBA", (A4_WIDTH, A4_HEIGHT), (255, 255, 255, 255))
    draw = ImageDraw.Draw(page)
    ...
    for i, j, _ in stickers(5, 7):
        if with_border:
            draw_circle(draw, i, j, diameter=DIAMETER, pitch=PITCH, margin_top=MARGIN_TOP, margin_left=MARGIN_LEFT)
        draw_text(draw, i, j, t[name(j)], font)

    page.save(f"./output/{language_code}_actions_text{'_b' if with_border else ''}.png")


def draw_discs(t: hash, language_code: str, font) -> None:
    draw_icon_discs(with_border=True)
    draw_icon_discs(with_border=False)

    draw_text_discs(t=t, font=font, language_code=language_code, with_border=True)
    draw_text_discs(t=t, font=font, language_code=language_code, with_border=False)
